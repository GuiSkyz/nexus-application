...
export function Dialog({ open, title, children, onClose, width = "md" }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="presentation" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()} className={`max-h-[calc(100vh-2rem)] w-full overflow-y-auto ${width === "lg" ? "max-w-2xl" : "max-w-lg"} rounded-xl bg-white p-6 shadow-overlay`}>
