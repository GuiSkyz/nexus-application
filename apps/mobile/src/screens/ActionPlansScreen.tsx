      ...
      <Modal visible={Boolean(selected)} transparent animationType="slide" onRequestClose={() => setSelected(undefined)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.modal}>
            ...
            {selected?.actionPlan ? <><Text style={styles.planLabel}>Ação solicitada</Text><Text style={styles.modalText}>{selected.actionPlan.description}</Text><Text style={styles.detail}>Responsável: {selected.actionPlan.assignedTo}</Text>{selected.actionPlan.assignedTechnicianId ? <Text style={styles.detail}>ID do técnico: {selected.actionPlan.assignedTechnicianId}</Text> : null}</> : <Text style={styles.waiting}>A gestão ainda não atribuiu uma ação corretiva.</Text>}
